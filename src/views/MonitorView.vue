<template>
  <div>
    ...
    <!-- 後端連線狀態 -->
    <div class="backend-bar" :class="monitor.backendConnected ? 'connected' : 'disconnected'">
      <span class="backend-dot"></span>
      <span v-if="monitor.backendConnected">後端伺服器已連線（port 3000）</span>
      <span v-else>後端伺服器未連線 — 請先執行 <code>cd server && npm start</code></span>
    </div>
    ...
    <transition name="fade">
      <div v-if="monitor.waitingCaptcha" class="captcha-alert">
        ...
        <div class="captcha-text">
          ...
          <div class="captcha-desc">請在已開啟的瀏覽器視窗中完成驗證，完成後點擊下方按鈕</div>
        </div>
        <button class="btn btn-success" @click="resolveCaptcha">✅ 我已完成驗證</button>
      </div>
    </transition>
    ...
    <div v-if="settings.eventUrl" class="card target-info">
      ...
      <div class="target-grid">
        ...
        <div class="target-item">
          ...
          <span class="target-val">{{ settings.ticketType || '未指定（自動選第一種）' }}</span>
        </div>
        <div class="target-item">
        ...
        <div class="target-item">
          ...
          <span class="target-val">{{ paymentData.cardNumber ? '信用卡 ****' + paymentData.cardNumber.slice(-4) : '未填（手動付款）' }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="log-container">
        ...
        >
          ...
          <span class="log-type-badge" :class="log.type">{{ typeLabel(log.type) }}</span>
...
.backend-bar.connected { background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.2); color: #4ade80; }
.backend-bar.disconnected { background: rgba(239,68,68,0.08); border: 1px solid rgba(239,68,68,0.2); color: #f87171; }
.backend-dot {
  width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0;
}
.connected .backend-dot { background: #22c55e; }
.disconnected .backend-dot { background: #ef4444; }
.backend-bar code { background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px; font-size: 12px; }
...
.captcha-icon { font-size: 28px; flex-shrink: 0; }
.captcha-text { flex: 1; }
.captcha-title { font-size: 15px; font-weight: 600; color: #fbbf24; }
.captcha-desc { font-size: 13px; color: #94a3b8; margin-top: 4px; }

.status-banner {
  display: flex; align-items: center; gap: 12px;
  padding: 14px 20px; border-radius: 10px;
  margin-bottom: 16px; border: 1px solid; transition: all 0.3s;
}
.status-banner.idle { background: rgba(100,116,139,0.1); border-color: #475569; }
.status-banner.running { background: rgba(124,106,255,0.1); border-color: #7c6aff; }
.status-banner.warn { background: rgba(234,179,8,0.1); border-color: #eab308; }
.status-banner.success { background: rgba(34,197,94,0.1); border-color: #22c55e; }

.status-dot {
  width: 12px; height: 12px; border-radius: 50%;
  background: #475569; flex-shrink: 0;
}
.status-banner.running .status-dot { background: #7c6aff; }
.status-banner.warn .status-dot { background: #eab308; }
...
.status-text { font-size: 15px; font-weight: 600; color: #e2e8f0; }
...
.metric-value { font-size: 28px; font-weight: 700; color: #7c6aff; }
.metric-label { font-size: 12px; color: #64748b; margin-top: 4px; }

.control-bar { display: flex; align-items: center; gap: 12px; margin-bottom: 20px; }
.btn-lg { padding: 12px 28px; font-size: 15px; }
.hint-text { font-size: 13px; color: #ef4444; }

.target-info { padding: 20px 24px; }
.target-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.target-item { display: flex; flex-direction: column; gap: 4px; }
.target-key { font-size: 12px; color: #64748b; }
.target-val { font-size: 14px; color: #e2e8f0; }
.url-text { word-break: break-all; color: #7c6aff; font-size: 12px; }

.log-count { font-size: 12px; color: #64748b; font-weight: 400; }
.log-container {
...
.log-container::-webkit-scrollbar-thumb { background: #2d3561; border-radius: 3px; }

.log-empty { text-align: center; padding: 40px; color: #64748b; font-size: 13px; }
.log-entry {
...
.log-type-badge {
  ...
  font-weight: 700; flex-shrink: 0; font-family: monospace;
}
.log-type-badge.info { background: rgba(59,130,246,0.2); color: #60a5fa; }
.log-type-badge.warn { background: rgba(234,179,8,0.2); color: #fbbf24; }
.log-type-badge.success { background: rgba(34,197,94,0.2); color: #4ade80; }
.log-type-badge.error { background: rgba(239,68,68,0.2); color: #f87171; }
